...
/** อัปโหลดรูปประกอบของรายการตรวจทีละรูป แล้วคืนเส้นทางไฟล์ให้ฟอร์มเก็บไว้ */
export async function POST(req: Request) {
  ...
  try {
    const path = newInspectionFilePath(photo.name || "photo.jpg");
    await uploadInspectionFile(path, await photo.arrayBuffer(), photo.type || "image/jpeg");
...
/** เปิดรูป — ส่งต่อไปยัง signed URL เฉพาะไฟล์ในโฟลเดอร์ inspection/ เท่านั้น */
export async function GET(req: Request) {
  ...
  if (!path || !path.startsWith("inspection/") || path.includes("..")) {
    return NextResponse.json({ error: "ไม่พบรูปนี้" }, { status: 400 });
  }

  const url = await inspectionFileUrl(path);
